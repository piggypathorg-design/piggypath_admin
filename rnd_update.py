...
new_add_block = """    schema.fields.forEach(field => {
    ...
    newBlock.teen.x = 20;
    newBlock.teen.y = 20 + (activeBlocks.length * 20);
    newBlock.teen.width = 300;
    newBlock.adult.x = 20;
    newBlock.adult.y = 20 + (activeBlocks.length * 20);
    newBlock.adult.width = 300;
    
    updatePagesWithActiveBlocks(blocks => [...blocks, newBlock]);"""

content = content.replace(old_add_block, new_add_block.replace('#', '//'))

# 3. Change canvas to remove flex flex-col and replace activeBlocks mapping
old_canvas = """              ) : (
...
if old_canvas in content:
    ...
    print("Warning: old canvas not found.")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("react-rnd update successful!")
